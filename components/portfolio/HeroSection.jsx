...
export default function HeroSection({
    ...
    const headline = personal?.title || "Information Technology Portfolio";
    ...
    const bio =
        personal?.bio || "Profile details and selected work will appear here.";
    const bioTypography =
        bio.length > 260
    ...
    const primaryAction = primarySection
        ? {
              href: `#${primarySection}`,
              label:
                  primarySection === "projects" ? "View Work" : "View Skills",
          }
        : null;
    ...
    return (
        <section
            id="hero"
            ref={sectionRef}
            className="overflow-visible bg-[#0B0D0C]"
        >
            <div className="relative min-h-[535px] overflow-hidden rounded-b-[1.25rem] bg-[#F7F5F2] shadow-[0_38px_85px_-18px_rgba(0,0,0,0.42)] sm:min-h-[calc(100svh-6.5rem)] sm:rounded-b-[1.5rem] lg:h-[calc(100svh-8rem)] lg:min-h-0 lg:rounded-b-[2rem]">
                ...
                >
                    ...
                    <span className="absolute right-5 top-[18%] font-mono text-xl font-bold leading-none text-[#FF3D00]/85">
                        x
                    </span>
                    <span className="absolute bottom-[31%] right-7 font-mono text-2xl font-bold leading-none text-[#FF3D00]/75">
                        x
                    </span>
                </div>
                <div className="relative z-10 mx-auto grid min-h-[535px] max-w-7xl grid-cols-1 px-5 pb-14 pt-12 sm:min-h-[calc(100svh-6.5rem)] sm:px-8 sm:pb-12 sm:pt-16 lg:h-full lg:min-h-0 lg:grid-cols-[minmax(0,0.62fr)_minmax(320px,0.38fr)] lg:items-center lg:px-12 lg:pb-0 lg:pt-0 xl:px-16">
                    <div className="relative z-20 w-full min-w-0 max-w-4xl lg:translate-y-[6%] lg:pl-[8%] xl:pl-[12%]">
                        ...
                        >
                            ...
                            <p
                                className={`mt-2 max-w-full break-words pr-1 font-medium text-[#3C403C] ${bioTypography}`}
                            >
                                {bio}
                            </p>
                        </div>
                            ...
                            {profileMeta
                                ...
                                .map((item, index, list) => (
                                    <React.Fragment key={`${item}-${index}`}>
                                        <span>{item.trim()}</span>
                                        {index < list.length - 1 && (
                                            <span className="mx-3 text-[#FF3D00]">
                                                /
                                            </span>
                                        )}
                                    </React.Fragment>
